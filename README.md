...
`v0.8.0`：主版本出版前稳定性候选版；60 款票库、逐格可复算的验奖引擎与真实 SVG 图符。

- 点击商场彩票店后直接进入成年确认与预算，再进入柜台；已撤下剧情状态机
...
- 预算严格限制为 5—10000 元的 5 元倍数，避免空值、越界或小数破坏资金状态
- 大模型与剧情均不接入当前主版本，避免额外状态导致兑奖链路故障
...
仓库内已包含原生微信小程序适配版，目录为 [`miniprogram/`](miniprogram/)；它不是网页 WebView，也不需要 ChatGPT 登录。当前小程序版本为 `0.1.1-mp`，具备进店、预算、选票、整本、逐步刮膜、柜台验奖和奖金换票的完整闭环。
...
`0.1.1` 已于 2026-08-08 上传为微信公众平台开发版本。后续版本可通过仓库内的官方上传工具发布，具体命令和密钥安全要求见迁移文档。
